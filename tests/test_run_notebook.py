...
def test_basic_into_caller_globals():
    runnb.run_notebook(FIXTURES / "basic.ipynb")
    g = globals()
    assert g["x"] == 42
...
def test_explicit_ns():
    ns = {}
    runnb.run_notebook(FIXTURES / "basic.ipynb", ns=ns)
    assert ns["x"] == 42
...
def test_line_magic_run():
    ns = {}
    runnb.run_notebook(FIXTURES / "with_magics.ipynb", ns=ns)
...
def test_chdir_default(tmp_path):
    ...
    try:
        os.chdir(tmp_path)
        ns = {}
        runnb.run_notebook(FIXTURES / "reads_local_file.ipynb", ns=ns)
...
def test_chdir_false(tmp_path):
    ...
    try:
        ...
        with pytest.raises(FileNotFoundError):
            runnb.run_notebook(FIXTURES / "reads_local_file.ipynb", chdir=False)
    finally:
        os.chdir(original_cwd)
...
def test_raise_on_error():
    ...
    with pytest.raises(ValueError, match="boom"):
        runnb.run_notebook(FIXTURES / "raises.ipynb", ns=ns)
    assert ns["before"] == 1
...
def test_keep_going():
    ns = {}
    runnb.run_notebook(FIXTURES / "raises.ipynb", ns=ns, raise_on_error=False)
    assert ns["before"] == 1
    assert ns["after"] == 2
...
def test_returns_ns():
    ns = {}
    result = runnb.run_notebook(FIXTURES / "basic.ipynb", ns=ns)
    assert result is ns
